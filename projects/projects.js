  ...
  arcData.forEach((arcDatum, idx) => {
    g.append("path")
      ...
      .on("click", () => {
        ...
        console.log(
          `You clicked on the slice for ${data[idx].label} (${data[idx].value} projects)`,
        );
        ...
        if (selectedIndex !== -1) {
          ...
          d3.select(event.target).classed("selected", true).attr("opacity", 1);

          d3.select(`.legend-${idx}`).classed("selected", true);
